window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var STYLE_TEXT = [
			...
			".dsh-ms-effort{display:flex;flex-direction:column;gap:4px;padding:6px 6px 2px;}",
			".dsh-ms-effortLabel{font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary);font-weight:500;letter-spacing:0.3px;text-transform:uppercase;}",
			...
			/* Chip — shown after model is selected, sits beside the trigger. */
			".dsh-ms-chip{display:inline-flex;align-items:center;gap:4px;height:24px;padding:0 8px;",
				"border-radius:12px;font-size:12px;font-weight:500;line-height:18px;",
				"background:var(--dsw-alias-interactive-bg-selected);",
				"color:var(--dsw-alias-label-primary);white-space:nowrap;user-select:none;}",
			".dsh-ms-chipDot{width:6px;height:6px;border-radius:50%;",
				"background:var(--dsw-alias-state-success-primary);flex:0 0 auto;}",
			".dsh-ms-chipEffort{color:var(--dsw-alias-label-tertiary);font-size:11px;}",
		].join("");
		...
		function ModelSearch(props) {
			...
			// Chip — show selected model + effort when a model is picked
			if (state.current) {
				var effortLabels2 = { low: "Low", medium: "Medium", high: "High", max: "Max" };
				var chipEffort = state.current.effort || effort;
				children.push(react.createElement("span", { key: "chip", className: "dsh-ms-chip", "aria-label": "Selected model: " + currentName + ", effort: " + effortLabels2[chipEffort] },
					react.createElement("span", { className: "dsh-ms-chipDot" }),
					react.createElement("span", null, currentName),
					react.createElement("span", { className: "dsh-ms-chipEffort" }, effortLabels2[chipEffort])
				));
			}
